...
{
    if (numprocs == 1) {
        neighbors_rank[0] = neighbors_rank[1] = neighbors_rank[2] = neighbors_rank[3] = MPI_PROC_NULL;
    }
    else 
    {
        neighbors_rank[0] = (grid[0] == 1 || (my_rank / grid[1]) == 0) ? MPI_PROC_NULL : my_rank - grid[1]; // top_rank
        neighbors_rank[1] = (grid[0] == 1 || (my_rank / grid[1] + 2) > grid[0]) ? MPI_PROC_NULL : my_rank + grid[1]; // bottom_rank
        neighbors_rank[2] = (grid[1] == 1 || (my_rank % grid[1]) == 0) ? MPI_PROC_NULL : my_rank - 1; // left_rank
        neighbors_rank[3] = (grid[1] == 1 || ((my_rank + 1) % grid[1]) == 0) ? MPI_PROC_NULL : my_rank + 1; // right_rank
    }
    // std::cout << "Process "<< my_rank << " - top_rank: " << neighbors_rank[0] << ", bottom_rank: "  << neighbors_rank[1] 
    //                         << ", left_rank: " << neighbors_rank[2]  << ", right_rank: " << neighbors_rank[3] <<  std::endl;
}